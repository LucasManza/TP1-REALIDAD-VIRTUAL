...
namespace VRTK.Examples {
    ...
    public class Openable_Door : VRTK_InteractableObject {
        ...
        protected void Start() {
            ...
            GetComponent<VRTK_InteractableObject_UnityEvents>().OnUse.AddListener((o, args) => tagBodyCollition());
        }
        ...
        private void SetDoorRotation(Vector3 interacterPosition) {
            side = ((rotated == false && interacterPosition.z > transform.position.z) ||
                    (rotated == true && interacterPosition.x > transform.position.x)
                ? -1
                : 1);
        }

        private void tagBodyCollition() {
            gameObject.tag = gameObject.CompareTag("NoCollition") ? "Untagged" : "NoCollition";
        }
    }
}